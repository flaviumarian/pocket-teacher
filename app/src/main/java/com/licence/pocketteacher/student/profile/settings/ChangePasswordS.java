...
public class ChangePasswordS extends AppCompatActivity {
    ...
    public void onBackPressed() {
        ...
        if (HelpingFunctions.isEditTextEmpty(passwordET) && HelpingFunctions.isEditTextEmpty(confirmPasswordET)) {
            finish();
            ChangePasswordS.this.overridePendingTransition(R.anim.anim_no_slide, R.anim.anim_slide_out_right);
            return;
        }
        ...
        String oldPassword = HelpingFunctions.getPasswordBasedOnUsername(MainPageS.student.getUsername());
        try {
            oldPassword = PasswordEncryptDecrypt.decrypt(oldPassword);

            if (!oldPassword.equals(passwordET.getText().toString())) {
                HelpingFunctions.setPasswordBasedOnUsername(MainPageS.student.getUsername(), PasswordEncryptDecrypt.encrypt(passwordET.getText().toString()).trim());
            } else {
